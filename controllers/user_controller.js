...
try {
  ...
  const returnOrder = await Returnorder.findOne({ userId: user,orderId,productId });
...
const filterShop= async (req, res) => {
    try {
        ...
        if (!currentlat || !currentlon) {
            return res.status(400).json({ "result": "false", "message": "required parameters are currentlat,currentlon ,rating,distances1,distances2" });
        }
		...
		for (const shop of shopList) {
		  const storeLat = shop.location.coordinates[1];
		  const storeLon = shop.location.coordinates[0];
		  const distance = parseFloat(calculateDistance(currentlat, currentlon, storeLat, storeLon).toFixed(1));
...
const usedGiftandPromotion_code=async(req,res)=>{
	try{
		...
		if(!userId || !code ){
			return res.status(400).json({result:"false","message":"Required fields are userId,code"})
		}
		
		const findData=await Gift.findOne({receiver_userId:userId,card_no:code});
		if(!findData){
		const findData1=await Promotion.findOne({cupon_code:code});
		if(!findData1){
			return res.status(400).json({result:"false","message":"Invalid code number"})

		}
        res.status(200).json({result:"true","message":" Promotion ode applied sucessfully",data:findData1});


		}else{
        await Gift.findOneAndUpdate({receiver_userId:userId,card_no:code},{status:1},{new:true})
		res.status(200).json({result:"true","message":"Gift code applied sucessfully",data:findData});
}
	}catch(err){
		res.status(400).json({result:"false","message":err.message})
	}
